...
export async function submitApplication(_: any, formData: FormData) {
  try {
    ...
    if (!nameRaw) return { success: false, message: "Please enter your full name." };
    if (!emailRaw || !isValidEmail(emailRaw)) {
      return { success: false, message: "Please enter a valid email address (example: name@gmail.com)." };
    }
    if (!phoneRaw) return { success: false, message: "Please enter your phone number." };
    if (!jobTitle || !jobId) return { success: false, message: "Job details missing. Please reopen the application form." };
    ...
    if (file && file.size > 0) {
      ...
      if (file.size > MAX) return { success: false, message: "Resume too large. Please upload under 5MB." };
    ...
    const { error } = await resend.emails.send({
      from: "Careers Form <onboarding@resend.dev>",
      to: ["kshreyas200129@gmail.com"],
    ...
    if (error) {
      ...
      return { success: false, message: "Unable to send application right now. Please try again in 1 minute." };
    }
